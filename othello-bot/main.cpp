...
std::optional<std::pair<int64_t,char>> minmax(DATA & d, int64_t depth, int64_t alpha, int64_t beta,bool isBlack,
                       std::chrono::time_point<std::chrono::steady_clock> start, uint64_t maxTime,int64_t depthToSave) {
	...
	if (depth == 0) {
		...
		return std::pair<int64_t, char>{ s,d.m_moveByte };
	}
	...
	if (d.m_children.empty()) {
		...
		if (children.empty()) {
			if (getLegitMoves(d.m_stav, !isBlack).empty()) {
				return std::pair<int64_t, char>{heuristika(d.m_stav),d.m_moveByte};
			}
			DATA c = d;
	...
	if (isBlack) {
		...
		for (DATA& child : (d.m_children.empty()) ? children : d.m_children) {
			...
			if (maxVal < ret.value().first) {
				maxVal = ret.value().first;
				retByte = child.m_moveByte;
			}
			alpha = std::max(alpha, ret.value().first);
			if (alpha >= beta) {
				break;
			}
		}
		...
		d.value = maxVal;
		return  std::pair<int64_t, char>{maxVal,retByte};
	}
	else {
		...
		for (DATA& child : (d.m_children.empty()) ? children : d.m_children) {
			...
			if (!ret.has_value()) return ret;
			if (minVal > ret.value().first) {
				minVal = ret.value().first;
				retByte = child.m_moveByte;
			}
			beta = std::min(beta, ret.value().first);
			if (alpha >= beta) {
				break;
			}
		}
		...
		d.value = minVal;
		return  std::pair<int64_t, char>{minVal,retByte} ;
	}
}
...
std::pair<char,uint64_t> iterativeSearch(DATA& d, bool isBlack, std::chrono::time_point<std::chrono::steady_clock> start, uint64_t maxTime,uint64_t lastDepth) {
	...
	while (depth != countALL+1) {
		...
		if (searchResult.has_value()) {
			//maxScore = searchResult.value().first;
			bestMove = searchResult.value().second;
		}
		if (!searchResult.has_value()) {
			depth--;
			break;
...
int main() {	
	...
	uint64_t maxCas = 0;
	bool started = false;
	bool color = false; //true - black; false - white
	std::string pos;
	pos.resize(3);
	DATA d;
	...
	while (std::getline(std::cin, command)) {
		...
		if (com.first == 1) { //START <B> <time>
			...
			color = (com.second[1] == "B") ? true : false; //:D
			try {
				maxCas = std::stoull(com.second[2]);
			}
			catch (...) {
				std::clog << "cant convert number" << std::endl;
				return EXIT_FAILURE;
			}
			if (maxCas == 0) {
				return EXIT_FAILURE;
